...
//
//namespace Eigen { typedef Matrix<double, 4, 1, DontAlign, 4, 1> VectorMR; }
//
...
//{
//public:
//	MultiRobot() { q_ << 0., 0., 0., 0.; }
//	MultiRobot(const Eigen::VectorMR &q) : q_(q) { }
//
...
//
//	Eigen::VectorMR& q() { return q_; }
//
//	static std::vector<fcl::Transform3f> ForwardKinematic(const Eigen::VectorMR &q);
//	static Eigen::VectorMR Random(std::mt19937_64 &rng, std::uniform_real_distribution<double> &unif);
//	static bool IsInsideRange(const Eigen::VectorMR &q);
//
...
//			template<> struct tag<MultiRobot> { typedef point_tag type; };
//			template<> struct coordinate_type<MultiRobot> { typedef double type; };
//			template<> struct coordinate_system<MultiRobot> { typedef cs::cartesian type; };
//			template<> struct dimension<MultiRobot> : boost::mpl::int_<4> {};
//			template<> struct access<MultiRobot, 0>
//			{
//				static double get(MultiRobot const& p) { return p[0]; }
//				static void set(MultiRobot& p, double const& value) { p[0] = value; }
//			};
//			template<> struct access<MultiRobot, 1>
//			{
//				static double get(MultiRobot const& p) { return p[1]; }
//				static void set(MultiRobot& p, double const& value) { p[1] = value; }
//			};
//			template<> struct access<MultiRobot, 2>
//			{
//				static double get(MultiRobot const& p) { return p[2]; }
//				static void set(MultiRobot& p, double const& value) { p[2] = value; }
//			};
//			template<> struct access<MultiRobot, 3>
//			{
//				static double get(MultiRobot const& p) { return p[3]; }
//				static void set(MultiRobot& p, double const& value) { p[3] = value; }
//			};
//		}
//	}
...
//#include "cell.inl"
//
//typedef Cell<MultiRobot, MyCell> MultiRobotCell;
//
//#endif
